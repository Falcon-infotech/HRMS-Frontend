...
const PayslipComponent = () => {
  ...
  const handleDownloadPDF = () => {
    if (payslipRef.current) {
      const options = {
        margin: 0.2,
        filename: `Payslip-${formData?.userId?.first_name}-${formData?.month}-${formData?.year}.pdf`,
  ...
  if (!formData) return <p className="text-center mt-10"><Loading text={"Loading payslip..."} /></p>;
    ...
    <div>
      <div className="flex justify-end mb-4">
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 flex gap-2"
        ><Download />
          Download PDF
        </button>
      </div>
      <div ref={payslipRef} className="max-w-full mx-auto border border-black p-4 bg-white text-sm print:text-xs mt-6">
        {/* Header */}
